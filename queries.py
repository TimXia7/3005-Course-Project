...
def Q_1(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT players.name, AVG(shots.xG)
        FROM players
            JOIN events ON players.id = events.player_id
            JOIN shots ON events.event_id = shots.event_id
            JOIN matches ON events.match_id = matches.id
            JOIN competitions ON matches.competition_id = competitions.competition_id AND matches.season_id = competitions.season_id
        WHERE 
            competitions.season_name = '2020/2021' -- no need to check anything else for this db, save some query time
        GROUP BY players.id 
        HAVING COUNT(shots.event_id) > 0
        ORDER BY AVG(shots.xG) DESC;
    """
...
def Q_2(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT players.name AS player_name, COUNT(shots.event_id) AS shot_count
        FROM players
            JOIN events ON players.id = events.player_id
            JOIN shots ON events.event_id = shots.event_id
            JOIN matches ON events.match_id = matches.id
        ...
        ORDER BY shot_count DESC, player_name;
    """
...
def Q_3(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT players.name, COUNT(shots.event_id) AS ft_shots
        FROM players
            JOIN events ON players.id = events.player_id
            JOIN shots ON events.event_id = shots.event_id
            JOIN matches ON events.match_id = matches.id
            JOIN competitions ON matches.competition_id = competitions.competition_id AND matches.season_id = competitions.season_id
        WHERE competitions.competition_name = 'La Liga' AND shots.first_time = true --only check competition_name as we want all La Liga seasons (save some query time!)
        GROUP BY players.name
        HAVING COUNT(shots.event_id) > 0
        ORDER BY ft_shots DESC, players.name; --since the sample queries for Q_1 and Q_2 seemed to use alphabetical
    """
...
def Q_4(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT teams.name, COUNT(passes.event_id) AS pass_count
        FROM teams
            JOIN players ON teams.id = players.team_id
            JOIN passes ON players.id = passes.recipient_player_id
            JOIN events ON passes.event_id = events.event_id
            JOIN matches ON events.match_id = matches.id
            JOIN competitions ON matches.competition_id = competitions.competition_id AND matches.season_id = competitions.season_id
        WHERE competitions.season_name = '2020/2021'
        GROUP BY teams.name
        HAVING COUNT(passes.event_id) > 0
        ORDER BY pass_count DESC, teams.name;
    """
...
def Q_5(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT players.name, COUNT(passes.event_id) AS pass_count -- this is just to rename the column 
        FROM players
            JOIN passes ON passes.recipient_player_id = players.id
            JOIN events ON passes.event_id = events.event_id
            JOIN matches ON events.match_id = matches.id
            JOIN competitions ON Matches.competition_id = competitions.competition_id AND matches.season_id = competitions.season_id
        WHERE competitions.season_name = '2003/2004'
        GROUP BY players.name
        HAVING COUNT(passes.event_id) > 0
        ORDER BY pass_count DESC, players.name;
    """
...
def Q_6(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT teams.name AS team, COUNT(*) AS number_of_shots FROM
        ...
        WHERE
            events.type = 'Shot' AND
            competitions.competition_name = 'Premier League' AND
            competitions.season_name = '2003/2004'
        GROUP BY teams.name
...
def Q_9(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT players.name AS player_name, COUNT(*) AS number_of_completed_dribbles FROM
            events JOIN dribbles ON events.event_id = dribbles.event_id
            JOIN players ON players.id = events.player_id
            JOIN matches ON events.match_id = matches.id
            JOIN competitions ON competitions.competition_id = matches.competition_id
        WHERE
            dribbles.complete = true AND
            competitions.competition_name = 'La Liga' AND
            (
            competitions.season_name = '2020/2021' OR
            competitions.season_name = '2019/2020' OR
            competitions.season_name = '2018/2019'
            )
        GROUP BY players.name
        ORDER BY number_of_completed_dribbles DESC;
    """
...
def Q_10(cursor, conn, execution_time):
    ...
    query = """ 
        SELECT players.name AS player_name, COUNT(*) AS number_of_dribbled_past  FROM
            events JOIN players ON events.player_id = players.id
            JOIN matches ON events.match_id = matches.id
            JOIN competitions ON competitions.competition_id = matches.competition_id
        WHERE
            events.type = 'Dribbled Past' AND
            competitions.competition_name = 'La Liga' AND
            competitions.season_name = '2020/2021'
        GROUP BY players.name
        ORDER BY number_of_dribbled_past ASC
    """
